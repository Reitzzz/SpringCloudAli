...
**本文档适用版本**：Spring Boot 3.2.14 + Spring Cloud 2023.0.1 + Spring Cloud Alibaba 2023.0.1.0
> ⚠️ 重要：遵循官方微服务组件发布对照表，避免版本列车跨代强扭导致的启动失败
...
* **服务调用：** Ribbon（属于*Netflix*，停止更新，已经彻底被移除）、SpringCloud LoadBalancer（属于*SpringCloud*官方，目前的默认方案，Spring Cloud 2024.0.0+ 已完全移除Ribbon）
...
2. **服务注册与发现**：适配 Spring Cloud 服务注册与发现标准，默认集成了 LoadBalancer 的支持（不再支持 Ribbon）。
...
> **推荐版本**：Nacos 2.3.2+ 与 Spring Cloud 2023.x 搭配使用，已验证稳定性。
...
可以看到目前最新的版本是`2.3.2`版本，我们直接下载`zip`文件即可。
...
<dependencyManagement>
    <dependencies>
        ...
        <dependency>
      	...
      	<!-- 这里引入最新的SpringCloudAlibaba依赖，2023.0.1.0版本与 Spring Cloud 2023.0.1 配套 -->
        <dependency>
...
<!-- LoadBalancer 作为默认负载均衡器（Ribbon 已移除） -->
<dependency>
    <groupId>org.springframework.cloud</groupId>
...
```xml
<!-- 现代微服务不再需要 bootstrap 相关依赖 -->
<!-- 直接在 application.yml 中使用 spring.config.import 加载远程配置 -->
<dependency>
    <groupId>com.alibaba.cloud</groupId>
...
在借阅服务的 `application.yml` 中进行配置（而非 bootstrap.yml）：
...
> **关键版本建议**：使用 Sentinel 1.8.8+ 稳定版本（不建议使用 2.0.0 beta）。1.8.x 系列已完美处理 Jakarta EE 命名空间变更，Servlet 流量拦截器工作稳定，市面上广泛使用。
...
> **在 Spring Boot 3.2.x 环境中**，推荐使用 Seata 1.7.0+ 稳定版本（测试版本 2.0.0 在 Bean 注入上仍有未知风险）。
...
Seata也是以服务端形式进行部署的，然后每个服务都是客户端，服务端下载地址：https://github.com/seata/seata/releases

**版本选择建议**：选择 1.7.0+ 的稳定释放版本而非测试版。
...
现在我们接着来配置开启分布式事务，首先在启动类添加注解，此注解会添加一个后置处理器将数据源封装为支持分布式事务的代理数据源（虽然官方表示配置文件中已经默认开启了自动代理，但是测试2.0.0版本下只能打注解的方式才能生效）：
